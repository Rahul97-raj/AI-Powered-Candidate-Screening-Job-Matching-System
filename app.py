...
if st.button("🚀 Screen Candidates", type="primary", use_container_width=True):
    ...
    for i, file in enumerate(uploads):
        try:
            ...
            comparison = compare_resume_to_job(profile, job_description)
            score = score_candidate(comparison)
    ...
    if not results:
        st.info("No candidates could be processed.")
        st.stop()
    ...
    table = pd.DataFrame([
        {
            ...
            "Match %": r["score"],
            "Matching Skills": len(r["comparison"].matching_skills),
            "Missing Skills": len(r["comparison"].missing_skills),
        }
    ...
    for rank, item in enumerate(results, 1):
        ...
        with st.expander(
            f"#{rank}  {item['filename']}  —  {item['score']}% Match",
            expanded=(rank == 1),
        ):
            a, b, d = st.columns(3)
            a.metric("Match Score", f"{item['score']}%")
            b.metric("Matching Skills", len(c.matching_skills))
            d.metric("Missing Skills", len(c.missing_skills))

            st.write(f"**Candidate:** {p.name or 'Not specified'}")
            st.write(f"**Email:** {p.email or 'Not specified'}")
            st.write(f"**Experience:** {p.experience_summary or 'Not specified'}")

            st.markdown("**Skills**")
            st.write(", ".join(p.skills) or "None extracted")

            col1, col2 = st.columns(2)
            with col1:
                st.markdown("**✅ Matching Skills**")
                st.write(", ".join(c.matching_skills) or "None")
            with col2:
                st.markdown("**❌ Missing Skills**")
                st.write(", ".join(c.missing_skills) or "None")

            st.markdown("**Recommendation**")
            st.info(c.recommendation or "No recommendation returned.")

st.caption("Portfolio/learning project — screening output should support, not replace, human hiring decisions.")
